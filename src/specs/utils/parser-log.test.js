...
const log = fs.readFileSync(path.resolve(__dirname, '../../../log/games.log'), 'utf-8');
...
describe('Testing class ParserLog', () => {
  test('Receive a file log, expected a return of data: [{ game_1: { total_kills: 0, players: [ \'Isgalamido\' ], kills: { Isgalamido: 0  } } }]', () => {
    ...
    var parserLog = new ParserLog(log);
      var games = parserLog.getGames();

    expect(games).toEqual(expect.arrayContaining(result));
