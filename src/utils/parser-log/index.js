...
const MOD_SHOTGUN = 'MOD_SHOTGUN';
const MOD_RAILGUN = 'MOD_RAILGUN';
const MOD_GAUNTLET = 'MOD_GAUNTLET';
...
class ParserLog {
  ...
  updateKilledBy(line) {
    const keyGame = this.getKeyGame();
    const [ids, names] = replaceLine(line, patterns.killers).split(': ');
    const modShotgun = names.includes(MOD_SHOTGUN);
    const modRailgun = names.includes(MOD_RAILGUN);
    const modGauntlet = names.includes(MOD_GAUNTLET);

      if (modShotgun) {
        this.killedBy.kills += 1;
        return;
      }else if (modRailgun) {
        this.killedBy.kills += 1;
    } else if (modGauntlet) {
